...
def take_note(note):
    ...
    time.sleep(1)  # Wait for Notepad to open (adjust as needed)

    # Create an instance of the SystemTasks class
    sys_task = SystemTasks()

    # Write the note
    sys_task.write(note)

    # Save the note with a random number appended to the filename
    sys_task.save(f'note_{randint(1, 100)}')

# def start_stopwatch():
#     start_time = time.time()
#     while True:
#         # Check for user voice command to stop the stopwatch
#         command = record_user_command()
#         if command and ("stop" in command or "pause" in command):
...
#         print("Listening for stopwatch commands...")
